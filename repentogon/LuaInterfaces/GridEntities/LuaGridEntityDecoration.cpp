...
LUA_FUNCTION(Lua_GridToGridDecoration) {
	GridEntity* grid = lua::GetUserdata<GridEntity*>(L, 1, lua::Metatables::GRID_ENTITY, "GridEntity");
	if (grid->GetDesc()->_type == 1) { //TODO: enum
		GridEntity_Decoration* newGrid = lua::UserdataToData<GridEntity_Decoration*>(lua_touserdata(L, 1));

		luaL_setmetatable(L, lua::metatables::GridDecorationMT);
	}
	else
		lua_pushnil(L);
	return 1;
...
void RegisterDecorationMetatable(lua_State* L) {
	lua::RegisterFunction(L, lua::Metatables::GRID_ENTITY, "ToDecoration", Lua_GridToGridDecoration);

	luaL_newmetatable(L, lua::metatables::GridDecorationMT); // meta
	...
	lua_pushstring(L, "GridEntityDecoration"); // meta __type GridEntityDecoration
	lua_rawset(L, -3); // meta

	lua_pushstring(L, "__class"); // meta __class 
	lua_pushstring(L, "GridEntityDecoration"); // meta __class GridEntityDecoration
	lua_rawset(L, -3); // meta
...
HOOK_METHOD(LuaEngine, RegisterClasses, () -> void) {
	...
	RegisterDecorationMetatable(_state);
}