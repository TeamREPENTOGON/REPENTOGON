...
LUA_FUNCTION(Lua_GridToGridTrapDoor) {
	GridEntity* grid = lua::GetLuabridgeUserdata<GridEntity*>(L, 1, lua::Metatables::GRID_ENTITY, "GridEntity");
	if (grid->GetDesc()->_type == 17) { //TODO: enum
		GridEntity_TrapDoor* newGrid = lua::UserdataToData<GridEntity_TrapDoor*>(lua_touserdata(L, 1));

		luaL_setmetatable(L, lua::metatables::GridTrapDoorMT);
	}
	else
		lua_pushnil(L);
	return 1;
...
void RegisterTrapDoorMetatable(lua_State* L) {
	lua::RegisterFunction(L, lua::Metatables::GRID_ENTITY, "ToTrapDoor", Lua_GridToGridTrapDoor);

	luaL_newmetatable(L, lua::metatables::GridTrapDoorMT); // meta
	...
	lua_pushstring(L, "GridEntityTrapDoor"); // meta __type GridEntityTrapDoor
	lua_rawset(L, -3); // meta

	lua_pushstring(L, "__class"); // meta __class 
	lua_pushstring(L, "GridEntityTrapDoor"); // meta __class GridEntityTrapDoor
	lua_rawset(L, -3); // meta
...
HOOK_METHOD(LuaEngine, RegisterClasses, () -> void) {
	...
	RegisterTrapDoorMetatable(_state);
}