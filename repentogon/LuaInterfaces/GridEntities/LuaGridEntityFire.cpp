...
LUA_FUNCTION(Lua_GridToGridFire) {
	...
	if (grid->GetDesc()->_type == 13) { //TODO: enum
		...
		luaL_setmetatable(L, lua::metatables::GridFireMT);
	}
	else
		lua_pushnil(L);
	return 1;
...
void RegisterFireMetatable(lua_State* L) {
	lua::RegisterFunction(L, lua::Metatables::GRID_ENTITY, "ToFire", Lua_GridToGridFire);

	luaL_newmetatable(L, lua::metatables::GridFireMT); // meta
	...
	lua_pushstring(L, "GridEntityFire"); // meta __type GridEntityFire
	lua_rawset(L, -3); // meta

	lua_pushstring(L, "__class"); // meta __class 
	lua_pushstring(L, "GridEntityFire"); // meta __class GridEntityFire
	lua_rawset(L, -3); // meta
