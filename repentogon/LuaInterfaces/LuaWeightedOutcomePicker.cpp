...
extern "C" {
	void L_WeightedOutcomePicker_AddOutcomeWeight(WeightedOutcomePicker* picker, uint32_t value, uint32_t weight) {
		WeightedOutcomePicker_Outcome outcome{ value, weight };
		picker->AddOutcomeWeight(outcome, false);
	}

	void L_WeightedOutcomePicker_AddOutcomeFloat(WeightedOutcomePicker* picker, uint32_t value, float weight, int scaleFactor) {
		WeightedOutcomePicker_Outcome outcome{ value, (uint32_t)(weight * scaleFactor) };
		picker->AddOutcomeWeight(outcome, false);
	}

	void L_WeightedOutcomePicker_ClearOutcomes(WeightedOutcomePicker* picker) {
	...
	void L_WeightedOutcomePicker_Free(WeightedOutcomePicker* picker) {
		delete picker;
	}

	FFI_OutcomeList L_WeightedOutcomePicker_GetOutcomes(WeightedOutcomePicker* picker) {
		auto& outcomes = *picker->GetOutcomes();

		FFI_OutcomeList list;
		list.outcomes = (WeightedOutcomePicker_Outcome**)malloc(outcomes.size() * sizeof(WeightedOutcomePicker_Outcome*));
		list.length = outcomes.size();

		for (int i = 0; i < outcomes.size(); i++) {
			list.outcomes[i] = &outcomes.at(i);
		}

		return list;
	}

	size_t L_WeightedOutcomePicker_GetNumOutcomes(WeightedOutcomePicker* picker) {
		auto& outcomes = *picker->GetOutcomes();
		return outcomes.size();
	}

	uint32_t L_WeightedOutcomePicker_PickOutcome(WeightedOutcomePicker* picker, RNG* rng) {
		return picker->PickOutcome(*rng);
	}

	void L_WeightedOutcomePicker_RemoveOutcome(WeightedOutcomePicker* picker, uint32_t value) {
		auto& outcomes = *picker->GetOutcomes();
