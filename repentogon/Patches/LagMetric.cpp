...
HOOK_METHOD(Entity_Laser, Update, (void)->void) {
	BitSet128& laser_flags = this->_tearFlags;
	int tris_flag = (1 << (60 - 32));
	...
	if ((laser_flags.lh & tris_flag) == tris_flag) {
		bool has_tris = (this->_parent && this->_parent->_type == 2 && ((((Entity_Tear*)(this->_parent))->_tearFlags.lh & tris_flag) == tris_flag));
		if (!has_tris) {
			goto skip_tris_check;
		};
