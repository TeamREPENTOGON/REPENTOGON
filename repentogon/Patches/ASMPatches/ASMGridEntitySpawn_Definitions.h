...
// and then end immediately after GridEntity::Init (or where the ai function would jump to immediately after this call)
const GridEntitySpawnPatchInfo patches[16] = {
	...
	{"8b0d????????8bbb",
	"Entity_NPC_Gideon::SpawnLadder", 0, GRID_STAIRS, NO_VARIANT_REG, 1, ASMPatch::Registers::EBX, 0xb9c, ASMPatch::Registers::EAX, 0, 0xa5},
	...
	"TrySpawnSanguineBondSpikes", 0, GRID_SPIKES, NO_VARIANT_REG, 100, ASMPatch::Registers::EBP, -0x14, ASMPatch::Registers::EBX, 0, 0x9b},

	{"8b4d??8945??85c978",
	"BombTearFlagEffects #1", 0, GRID_PIT, NO_VARIANT_REG, 0, ASMPatch::Registers::EBP, -0x4c, ASMPatch::Registers::EAX, 0, 0xb3},

	{"8985????????85f678??81fec00100007c??68????????6a03e8????????83c40885f60f88????????81fec0010000",
	"BombTearFlagEffects #2 (giga bomb)", 0, GRID_PIT, NO_VARIANT_REG, 0, ASMPatch::Registers::ESI, 0, ASMPatch::Registers::EAX, 0, 0xb6},

	// seems like this inline jumps partway through to a second inline, hence the big jmp
	{"81febf0100000f87????????50",
	"BombDamage (isaac's room staircase)", 0, GRID_STAIRS, NO_VARIANT_REG, 0, ASMPatch::Registers::ESI, 0, ASMPatch::Registers::EBP, -0x20, 0x19a},

	{"81febf01000077??50",
	"BombDamage (barren room trapdoor)", 0, GRID_TRAPDOOR, NO_VARIANT_REG, 0, ASMPatch::Registers::ESI, 0, ASMPatch::Registers::EBP, -0x20, 0x85},
};