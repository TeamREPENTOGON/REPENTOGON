...
void ASMPatchRoomSaveState(void* addr) {
	...
	patch.PreserveRegisters(reg)
		...
		.AddBytes("\x84\xc0") // TEST AL, AL
		.RestoreRegisters(reg)
		.AddConditionalRelativeJump(ASMPatcher::CondJumps::JZ, (char*)addr + 0x89) // jump if false
...
void ASMPatchDeepGaperClearCheck(void* addr) {
	...
	patch.PreserveRegisters(reg)
		...
		.AddInternalCall(CheckDeepGaperClearDelay) // call CheckSaveStateClearDelay()
		.AddBytes("\x84\xc0") // TEST AL, AL
		.RestoreRegisters(reg)
		.AddConditionalRelativeJump(ASMPatcher::CondJumps::JNZ, (char*)addr + 0x27) // jump if false
		.AddRelativeJump((char*)addr + 0xf); // jump to roomdescriptor flag check
	sASMPatcher.PatchAt(addr, &patch);
}
