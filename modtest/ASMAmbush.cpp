...
void PatchHardcodedAmbushWaveCountOLD() {
    printf("[REPENTOGON] ASM patch: hardcoded wave count\n");
    ...
    // First, we allocate memory for our new ASM to live in
    void* region = VirtualAlloc(NULL, 0x1C, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (region == NULL) {
        throw std::runtime_error("Unable to allocate memory\n");
    }
	...
	char _code[] = {
		// xor ebx, ebx (Blank EBX. Might not be necessary since we MOV instead of ADD like vanilla, kept for now since I know this works.)
		0x33, 0xDB, // 0x2
		// cmp eax, 1 (Check against the pre-set comparison.)
		0x83, 0xF8, 0x01, // 0x5
		// jne XXXXXXXX (If we're not a boss challenge room, jump 0xA bytes ahead.)
		0x75, 0x0A, // 0x7
		// mov ebx, 2 (Move the value 2 to the EBX register.)
		0xBB, 0X02, 0, 0, 0, // 0xC
		// jmp isaac-ng.XXXXXXXX (Jump to the original code.)
	...
	// Allocate new memory, and copy code to our block.
	char* code = (char*)malloc(sizeof(_code));
	memcpy(code, _code, 0x1C);

	// Copy the pointer address to the proper position in the block.
	memcpy(code + 0x17 - 4, &ptr, sizeof(ptr));

	// Copy the JMP address to the proper positions in the block.
	// We need to jump 0xB past our original block to bypass the vanilla calculation properly.
	void* offset = (char*)addr + 0xB;
	ptrdiff_t jmp = (char*)offset - ((char*)region + 0x11);
	ptrdiff_t jmp2 = (char*)offset - ((char*)region + 0x1C);

	memcpy(code + 0x11 - 4, &jmp, sizeof(jmp));
	memcpy(code + 0x1C - 4, &jmp2, sizeof(jmp2));
