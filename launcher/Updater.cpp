...
void DoAsyncUpdate() {
    ...
    if (curl) {
        ...
        // If the release name is different from our version name, we are out of date!
            ...
            if (MessageBoxA(NULL, message, "REPENTOGON", MB_YESNO | MB_SETFOREGROUND | MB_TOPMOST | MB_TASKMODAL) == IDYES) {
                ...
                curl_easy_cleanup(curl);
                sLogger->Info("dsound: CheckForUpdates: Finished downloading\n");
                iniFile.write(ini);
...
void CheckForUpdates() {
    ...
    asyncupd=std::thread(DoAsyncUpdate);
    asyncupd.detach();
    printf("Running async download!\n");
    return;
};